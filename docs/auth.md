...
If no keyring backend is available (e.g. a headless CI machine), or a
registered backend is present but non-functional (e.g. a broken D-Bus Secret
Service), tokens fall back to a JSON file:
