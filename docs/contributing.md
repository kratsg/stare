...
10. Add a live integration test in `tests/integration/test_live.py` (runs with
    `pixi run test-slow`)
11. Add representative fixture JSON under `tests/fixtures/` and load it in
    `tests/test_client.py` (model tests in `tests/test_models.py` keep
    hand-built dicts inline, not fixture files)
...
- `alias_generator=to_camel` — automatically maps `snake_case` fields to
  `camelCase` API keys (so explicit `Field(alias=...)` is only needed for
  non-standard names)
- `populate_by_name=True` — allows access by both alias and Python attribute
  name
- `model_validate()` override — wraps `ValidationError` in `ResponseParseError`
...
| `tests/test_models.py`           | Pydantic model parsing from hand-built dicts (no fixture JSON — endpoints are planned) |
