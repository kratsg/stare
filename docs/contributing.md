...
The library uses **httpx** for HTTP, **pydantic** for all data models,
implements OAuth2 PKCE natively, and **typer + rich** for the CLI.
...
9. Make it pass
10. Add a live integration test in `tests/integration/test_live.py` (runs with
    `pixi run test-slow`)
11. Add representative fixture JSON under `tests/fixtures/` and load it in the
    model tests
